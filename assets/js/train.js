...
const {encoder, decoder, ae, aeLoss} = require('./autoencoder');

let epochs;
let batchSize;

const INTERMEDIATE_DIM = 128;
const LATENT_DIM = 1;
 ...
 */
async function train(images, aeOpts, savePath, logDir) {
  const encoderModel = encoder(aeOpts);
  const decoderModel = decoder(aeOpts);
  const aeModel = ae(encoderModel, decoderModel);
